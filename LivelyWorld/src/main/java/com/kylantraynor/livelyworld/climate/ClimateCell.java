...
public class ClimateCell extends VCell{
	...
	public double getSunRadiation(){
		return Planet.getPlanet(world).getSunRadiation(new Location(world, getSite().getX(), 255, getSite().getZ()));
	}
	...
	public double getVolume(){
		if(!Double.isNaN(airVolume)) return airVolume;
		double volume = 0;
		for(VTriangle t : getTriangles()){
			VectorXZ a = t.points[0];
			VectorXZ b = t.points[1];
			VectorXZ c = t.points[2];
			volume += (a.getX() * (b.getZ() - c.getZ()) + b.getX() * (c.getZ() - a.getZ()) + c.getX() * (a.getZ() - b.getZ())) * 0.5;
		}
	...
	public double getAmount(){
		if(!Double.isNaN(airAmount)) return airAmount;
		airAmount = ClimateUtils.getGasAmount(getBasePressure(), getVolume(), getBaseTemperature());
		return airAmount;
	}
	
	public double getLowAltitudePressure(){
		if(!Double.isNaN(lowAltitudePressure)) return lowAltitudePressure;
