...
public class ClimateChunk {
	...
	public static ClimateChunk getAt(Location l){
		...
		for(ClimateChunk c : cachedChunks){
			if(c.getX() == chunk.getX() && c.getY() == y && c.getZ() == chunk.getZ()){
				return c;
			}
		}
	...
	public int getX() { return chunk.getX(); }
	public int getZ() { return chunk.getZ(); }
	
	public List<Location> getCorners(){
		if(cachedCorners != null) return cachedCorners;
		List<Location> corners = new ArrayList<Location>();
		corners.add(new Location(chunk.getWorld(), getX() * 16, getY() * 16, getZ() * 16));
		corners.add(new Location(chunk.getWorld(), getX() * 16 + 15, getY() * 16, getZ() * 16));
		corners.add(new Location(chunk.getWorld(), getX() * 16, getY() * 16 + 15, getZ() * 16));
		corners.add(new Location(chunk.getWorld(), getX() * 16, getY() * 16, getZ() * 16 + 15));
		corners.add(new Location(chunk.getWorld(), getX() * 16 + 15, getY() * 16 + 15, getZ() * 16));
		corners.add(new Location(chunk.getWorld(), getX() * 16 + 15, getY() * 16, getZ() * 16 + 15));
		corners.add(new Location(chunk.getWorld(), getX() * 16 + 15, getY() * 16 + 15, getZ() * 16 + 15));
		cachedCorners = corners;
		return corners;
	}
	
	public Temperature getAverageTemperature(){
		if(cachedAverageTemperature != null) return cachedAverageTemperature;
			...
			for(int y = 0; y < 16; y++){
				for(int z = 0; z < 16; z++){
					...
					if(current.getBlock().getType() != Material.AIR){
						lastTemp = planet.getClimate(current).getAverageTemperature().getValue();
						temp += lastTemp;
						count++;
					} else {
						lastTemp = (lastTemp + planet.getDefaultAirTemperature(current).value)/2;
						temp += lastTemp;
						count++;
					}
				}
		...
		for(int x = 0; x < 16; x++){
			for(int y = 0; y < 16; y++){
				for(int z = 0; z < 16; z++){
					...
					current.setZ(getZ() * 16 + z);
					if(current.getBlock().getType() != Material.AIR){
						lastTemp = planet.getClimate(current).getTemperature().getValue();
						temp += lastTemp;
						count++;
					} else {
						lastTemp = (lastTemp + planet.getDefaultAirTemperature(current).value)/2;
