...
public class Planet {
	...
	private boolean isAffectedWorld(World w) {
		if(this.mainWorld.equals(w)) return true;
	...
	public long getR(){ return getRadius(); }
	public double getC(){ return getCircumference(); }
	public double getOb(){ return getObliquity(); }
	...
	public double getObliquity(){
		return getInclination() * (-Math.cos( 2 * Math.PI * ((double)getDaysSinceIRLJanuary()/(double)getDaysInIRLYear())));
	}
	...
	public double getSunAverageRadiation(Location l){
		return Math.max(Math.cos((l.getZ() + getCurrentOffset()) / getR()), 0) * (l.getBlock().getLightFromSky() / 15.0);
	}
