...
public class CreaturesModule {
	...
	public void onBlockUpdate(Block b, Player p) {
		...
		if(b.getChunk().getEntities().length >= Bukkit.getServer().getMonsterSpawnLimit()) return;
		while(!b.isLiquid()){
			if(b.getY() <= 48){
		...
		if(b.isLiquid() && (b.getBiome() == Biome.OCEAN || b.getBiome() == Biome.DEEP_OCEAN)){
			...
			if(b.getLocation().distanceSquared(p.getLocation()) > 400){
				...
				if(b.getBiome() == Biome.DEEP_OCEAN){
					if(Math.random() * 100 <= 20){
						b.getLocation().getWorld().spawnEntity(b.getLocation(), EntityType.GUARDIAN);
					} else {
						b.getLocation().getWorld().spawnEntity(b.getLocation(), EntityType.SQUID);
					}
				} else {
					b.getLocation().getWorld().spawnEntity(b.getLocation(), EntityType.SQUID);
				}
