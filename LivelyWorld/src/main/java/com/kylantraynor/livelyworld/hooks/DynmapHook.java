...
public class DynmapHook {
	...
	private void loadWeatherMarkerSet() {
		...
		if (weatherSet == null) {
			weatherSet = markerAPI.createMarkerSet("livelyworld.markerset.weather", "Weather", null, false);
		} else {
	...
	public void updateClimateCell(ClimateCell c){
		...
		String weatherMarker = "weather_" + c.getWeather().toString().toLowerCase();
	    ...
	    if (weatherMarker != null)
	    {
	        ...
	        if (weatherIcon == null)
	        {
	          Bukkit.getServer().getLogger().info("Invalid Weather Icon: " + weatherMarker);
