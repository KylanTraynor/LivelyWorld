...
public class SoundManager {
		...
		ambiantSoundsTask = new BukkitRunnable(){
			...
			public void run() {
				for(Player p : Bukkit.getOnlinePlayers()){
					...
					int ry = (int) Math.max(Math.min((Math.random() - 0.5) * 50, 255), 0);
	...
	public void playAmbiantSound(Biome b, Location l){
		...
		case BEACHES:
			l.setY(48);
			if(l.getBlock().getType() == Material.SAND || l.getBlock().isLiquid()){
		...
		case RIVER:
			...
			break;
		case FOREST: case BIRCH_FOREST: case JUNGLE:
			if(l.getBlock().getType() == Material.LEAVES || l.getBlock().getType() == Material.LEAVES_2){
	...
	private void playSound(String string, Location l, float volume, float pitch) {
		for(Player p : Bukkit.getOnlinePlayers()){
			if(p.getWorld().equals(l.getWorld())){
				//plugin.log(Level.INFO, "Playing " + string + " to " + p.getName() + " from " + l.toString());
