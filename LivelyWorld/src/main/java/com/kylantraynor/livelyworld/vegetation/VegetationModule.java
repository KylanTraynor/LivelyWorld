...
public class VegetationModule implements Listener {
	...
	public void onBlockUpdate(Block b, Player p) {
		if(p != null){
			if(b.getLocation().distanceSquared(p.getLocation()) < 25) return;
		}
		...
		switch(b.getType()){
		case SAPLING: case LOG: case LOG_2: case LEAVES: case LEAVES_2:
		case FENCE: case DARK_OAK_FENCE: case SPRUCE_FENCE: case ACACIA_FENCE: case BIRCH_FENCE:
		case JUNGLE_FENCE:
			updateTree(b);
			break;
		case GRASS:
			if(Math.random() <= 0.5){
				updateGrass(b); } else { updateFlower(b);
			}
			break;
		case LONG_GRASS:
	...
	private void updateFlower(Block b) {
		...
		switch(rand){
		case 0: tryPlantDandelion(above); break;
		case 1: tryPlantPoppy(above); break;
		case 2: tryPlantOxeyeDaisy(above); break;
		default:
	...
	private void tryPlantDandelion(Block b) {
		...
		if(Planet.getPlanet(b.getWorld()) != null){
			...
			isClimateOk = (averageTemp.getValue() > 15.0 + 273.15 && averageTemp.getValue() < 25.0 + 273.15);
		}
		if(isClimateOk){
	...
	private void tryPlantPoppy(Block b) {
		...
		if(Planet.getPlanet(b.getWorld()) != null){
			ClimateChunk c = ClimateChunk.getAt(b.getLocation());
			Temperature averageTemp = c.getAverageTemperature();
			isClimateOk = (averageTemp.getValue() > 10.0 + 273.15 && averageTemp.getValue() < 15.0 + 273.15);
		}
		if(isClimateOk){
			if(debug)Bukkit.getServer().getLogger().info("Climate is Ok");
	...
	private void tryPlantOxeyeDaisy(Block b) {
		...
		if(Planet.getPlanet(b.getWorld()) != null){
			ClimateChunk c = ClimateChunk.getAt(b.getLocation());
			Temperature averageTemp = c.getAverageTemperature();
			isClimateOk = (averageTemp.getValue() > 10.0 + 273.15 && averageTemp.getValue() < 25.0 + 273.15);
		}
		if(isClimateOk){
			if(debug)Bukkit.getServer().getLogger().info("Climate is Ok");
	...
	private boolean isMaterialBelow(Location l, Material m, byte data, int depth){
		...
		while(i < depth && currentLocation.getBlockY() > 0){
			...
			if(currentLocation.getBlock().getType() == m){
				if(currentLocation.getBlock().getData() == data) return true;
			}
			i++;
		}
		return false;
